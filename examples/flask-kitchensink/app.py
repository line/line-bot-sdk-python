...
def handle_text_message(event):
    ...
    if text == 'profile':
        ...
        response = line_bot_api.get_number_of_message_deliveries(var_date=today)
        ...
        today = datetime.date.today().strftime("%Y%m%d")
        response = line_bot_api.get_number_of_followers(var_date=today)
        if response.status == 'ready':
            messages = [
        ...
        response = line_bot_api.get_friends_demographics()
